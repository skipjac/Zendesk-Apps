  (function () {
    ...
    var show = function() {
      $.each(arguments, function(i, item) {
        _.each(item, function(i){$('[name="' + i +'"]').parents('.form-field').css('display', '')});
      });
    };
    //hide fields
    var hide = function(hide, show) {
      hide.forEach(function(field){
         if (!_.contains(show, field)) {
          clearValue(field);
          $('[name="' + field +'"]').parents('.form-field').css('display', 'none');
         }
       }, this);
    };
    ...
    var clearValue = function(field){
      ...
      if($('[name="' + field +'"]').parent().hasClass('text')){
        console.log('this is a text field', field);
        $('[name="' + field +'"]').val('');
      }
      $('[name="' + field +'"]').parents('.nesty').children('.nesty-input').map(function(x,y){$(y).text('');});
    };
